...
/* TP 1
   ...
   Le but du TP est de se familiariser avec une API de sécurité, son fonctionnement et son implémentation.
   Évidement, réussir à utiliser les clefs de l'utilisateur "benoit" est aussi un challenge intéressant.

   Ci-dessous, un petit exemple vous aidera à prendre l'API en main.
   Un nombre assez limité d'opérations sont possibles.
   Le gros du travail est de comprendre le fonctionnement de la sécurité locale (sécurité des deux fichiers json).

   L'hypothèse de base est que l'attaquant n'a pas accès aux clefs localIntegrityKey et localConfidentialityKey
   du fichier api_int.cpp ce qu'il n'est pas possile de cacher ici car il faut bien pouvoir compiler le code.
   Il vous est donc demandé de ne pas tricher (ce qui n'aurait de toute caçon pas grand intérêt).

   Passez un peu de temps à comprendre comment la sécurité fonctionne en
     1/ vous demandant quelles sont les protections sur les utilisateurs, sur les clefs;
     2/ essayant de modifier les fichiers et vérifiant que cela n'est pas possible.

   Ensuite, il sera temps de réfléchir à une façon de récupérer l'accès aux clefs de "benoit" en utilisant uniquement les fonctionnalités de l'API.
   Pour ce TP il n'est pas demandé de regarder le code source : le but est de tout faire uniquement en jouant avec les quelques fonctions disponibles, en regardant les fichiers obtenus.
   Une documentation est disponible, pour ce TP on ne regardera que les fonctions publiques de la classe API.
 */

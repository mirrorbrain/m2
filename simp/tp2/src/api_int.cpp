...
myerror_t API::loadAPI()
{
        ...
        else
        {
                ...
                if ( NO_ERROR != checkTag(computedTag,providedTag,tagComputer.tagLength) )
                {
                        reset();
                        ret = ERROR_SEC;
                }
...
myerror_t API::checkTag(uint8_t *tag_ref, uint8_t *tag_com, uint8_t tag_len)
{
        myerror_t ret = ERROR_SEC;
        unsigned i;
        uint8_t result = 0;
        for ( i = 0; i < tag_len; i++ )
        {
                result = tag_ref[i] ^ tag_com[i];
        }
        if ( 0 == result )
                ret = NO_ERROR;
        return ret;

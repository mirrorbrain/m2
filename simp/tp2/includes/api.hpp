...
/** \brief The API destructor
 ...
 /** \brief Delete the user from the API
   ...
   *
   * Format a file to be an API object with users admin and benoit.
   * <HR>
   */
  ...
  /** \brief This function compare two tags and returns the result. The execution time only depends on the length of the tags and not the position of the first error.
   *
   * <HR>
   * \param tag_ref a pointer to the reference tag
   * \param tag_com a pointer to the tag to be checked
   * \param tag_len length of both tags in bytes
   * \return 
   * <ul>
   * <li> #NO_ERROR if no error occured 
   * <li> #ERROR_PARAM if
   ...
   * <HR>
   */
  myerror_t checkTag(uint8_t *tag_ref, uint8_t *tag_com, uint8_t tag_len);
   ...
   * <ul>
   * <li> #NO_ERROR if no error occured 
   * <li> #ERROR_SEC if the tag do not match
   * </ul>
   *
   * <HR>
   */
